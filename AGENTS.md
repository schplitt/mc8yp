...
src/
  ...
  codemode/
    ...
    external-mcp-session.ts   Per-session tool-list cache for connection-supplied MCP servers (15-min idle TTL)
    session-eviction.ts       Server-only: InfoSessionManager wrapper evicting a session's sandbox + external tool lists on clean client DELETE
    ...
    sandbox/
      types.ts                Swappable `SandboxAdapter` contract (Flue `SandboxApi` shape) + `SandboxFileStat`
      just-bash-adapter.ts    just-bash backend: in-memory shell + virtual FS, core shell only (no network/python/js)
      index.ts                Per-session store (keyed by ctx.sessionId) + 15-min idle eviction, `sandbox` host-module leaf (HostModuleObject), mtime→epoch-ms conversion
  ctx/
...
1. `src/cli/index.ts` parses CLI arguments and access-policy flags (`-r`, `--restrict`, `--restriction`, `-a`, `--allow`, and `--allowed`), plus the repeatable `--mcp-server` flag carrying one external MCP server as JSON (a malformed entry is a startup error).
...
- The runtime uses [`@iso4/sandbox`](https://www.npmjs.com/package/@iso4/sandbox) (>= 0.6.0) — a V8-isolate sandbox running in a separate Rust subprocess — with a 24 MB memory limit (kept small because the isolate heap ceiling counts against iso4's container admission line derived from the microservice's 512M cgroup limit), 50 s CPU / 120 s wall time, and 200 bridge calls per run. A single lazy-initialised `Sandbox` is shared across all `codemode` calls; iso4 multiplexes concurrent runs onto shared connections and spreads them over its own isolate slots.
- **`createSandbox()` takes no options and must stay that way.** Since 0.6.0 the runtime derives its concurrency from the shape of the runs it is serving and follows the workload; pinning `maxConcurrentRuns` freezes that at a number measured on someone else's hardware, and an explicit value that promises more concurrent heap than the container can hold earns a stderr warning from iso4. The same applies to `hostReserveMb` (the host's share of the container limit, 128 MB by default) and `memoryBudgetMb`: the defaults are container-aware, and mc8yp's own resource limits are the per-run `SANDBOX_LIMITS` in `execute.ts`, not runtime-wide capacity knobs.
...
- `createLiveCalls` takes a `TenantTransport` that is EITHER a resolved tenant transport OR `{ error }`. Tenant-bound dispatch (`operation`, tenant MCP tools) throws that message; external MCP namespaces still dispatch, because they carry their own credentials and never touch the tenant. That is why a CLI session with no active tenant can still call an external MCP server.
...
- **PROVENANCE, however, IS visible** — the one deliberate carve-out from the rule above. `codemode.describe()` labels an external namespace `EXTERNAL MCP server at <url> — configured for this connection, NOT part of this tenant`, and the method-level describe repeats it ("calls go to that host with its own credentials; tenant credentials are never sent"). OpenAPI-vs-MCP is an implementation detail the agent cannot act on; tenant-vs-third-party is a trust and data boundary it must be able to report on. The method-level repetition is not redundant: search can take the agent straight to a method without it ever reading the overview. Search results stay uniform (`namespace` is enough to map a hit back to a labelled namespace).

### External (Connection-Supplied) MCP Servers

Servers the CONNECTION brings, not the tenant: `mc8yp-mcp-server` header (repeatable, one JSON object per entry, or a JSON array in one header) and the CLI's repeatable `--mcp-server`. Entry shape `{ name, url, token?, headers?, description? }`.

- `src/utils/external-mcp.ts` owns collection, parsing/validation, the auth-header shape (`token` → `Authorization: Bearer <token>`, then `headers` so an explicit entry can override), and `createExternalMcpFetch` (plain `fetch` against the absolute URL). It is the analogue of `restrictions.ts` for this option and is deliberately a separate file — none of it is path-rule parsing.
- **Header-only, no query parameter.** Every other connection option also accepts a query param; this one does not, because the value carries a bearer token and query strings land in access logs and proxy traces. Do not "complete the pattern" by adding one.
- **Fail loud on malformed entries**: 400 in server mode, startup error in CLI. Skipping silently would leave the agent without a namespace the operator believes it has, with nothing pointing at the typo. A well-formed but UNREACHABLE server is different: it is reported to the agent in the `codemode.describe()` overview (`externalFailures` → `renderOverview`) and retried on the next call.
- **Tool lists come from `src/codemode/external-mcp-session.ts`**, cached per `(sessionId, config)` with a 15-minute idle TTL — same shape and same three eviction paths as the sandbox store (idle timer, clean client DELETE, process exit). Rotating a token or changing a URL changes the cache key, so it re-handshakes. Failures are never cached. Without this cache, every codemode call would pay `initialize` + `tools/list` per external server.
- **Namespace assembly appends them last and never lets them displace a tenant namespace** (name collision → skipped with a warning). `parseExternalMcpServers` additionally rejects `RESERVED_NAMESPACES` outright, so a header cannot impersonate `c8y`/`sandbox`/`codemode`/`docs`. Names are validated as JS identifiers rather than sanitized, so the agent-visible global is exactly what was configured.
...
- `codemode.describe` resolves at three altitudes: no target → namespace overview (`kind: 'overview'`), `"<ns>"` (or `"<ns>."`) → a one-line-per-method **listing** (`kind: 'namespace'`), `"<ns>.<method>"` or a bare method name → full types (`kind: 'method'`). Anything else redirects to search. A namespace name wins over a method that happens to share it; the method stays reachable as `<ns>.<method>`.
- Listing lines carry the **fully-qualified callable target** (`c8y.getAlarmCollectionResource`, `asset_svc.get_assets`) — the same string `codemode.search` returns as `target` and `describe` accepts, so the agent never has to reassemble the callable name from a bare method name plus the header. It costs ~4 chars × 251 lines on core.
- Beyond the target, a listing is deliberately **`METHOD /path` + `summary` only — never `description`**. That is the cost control: all 251 core operations render in ~30k chars (~7.5k tokens), where 251 full typed blocks would flood context. `test/real-spec.test.ts` pins both the one-line-per-operation invariant and a 40k-char budget; if a format change blows past it, change the format rather than the number. OpenAPI listings are grouped by each operation's FIRST declared tag (in order of first appearance, so every method appears exactly once) because the spec's own tags are the domain structure the agent is looking for — the group names double as `docs.read("<ns>::topic::<group>")` ids. MCP namespaces list flat (tools carry no tags) and their tool descriptions are clipped to 200 chars (`MCP_LISTING_DESCRIPTION_LIMIT`) rather than the 220 used for namespace descriptions — third-party servers routinely ship multi-paragraph tool prose where an OpenAPI `summary` is one line, so one server must not be able to dominate a listing. The clip is listing-only; `describe("<ns>.<tool>")` still renders the full description. Usage hints render ABOVE the listing: the agent must read how to go deeper before reading hundreds of lines.
- `namespaceHeadline` is shared by the overview and the listing header on purpose — the external-MCP provenance label is a trust boundary and must not drift between the two surfaces. The listing repeats the full "tenant credentials are never sent" note for external namespaces, for the same reason the method-level render does: search can take the agent straight there without it ever reading the overview.
...
**Server mode only, and disabled by default.** A non-tenant compute surface for agent data-wrangling, deliberately isolated from the live request path: no network, no host filesystem, never reaches Cumulocity. In CLI mode the `sandbox` global does not exist at all — local agent harnesses bring their own file I/O, so it would be dead weight. `execute()` builds it only when `custom.env === 'server'`, a `ctx.sessionId` is present, AND the connection has opted in via `custom.enableSandbox`; otherwise `buildApiModule` omits the key and the global is absent.

- **Opt-in, not opt-out.** The `mc8yp-enable-sandbox` header or `enableSandbox` query param (`collectServerEnableSandboxSources` / `parseEnableSandbox` in `src/utils/restrictions.ts`, same source file as the restriction/allow/noMcp parsing) enables the sandbox for a connection; without it, no `sandbox` global exists. Parsed once per request in `src/index.ts` into `ctx.custom.enableSandbox` (a plain boolean — unlike `noMcp` there is no per-service scoping, since a connection has exactly one sandbox), and read at the `buildSandboxApi` call site in `execute.ts`. This is a deliberate breaking change from an earlier design where the sandbox was built by default (with an opt-out): a general-purpose shell is a bigger blast radius than any single typed API call, so a deployment must explicitly turn it on rather than explicitly turn it off.
...
- **One sandbox per MCP session, idle-evicted.** `index.ts` keeps a `Map<sessionId, { adapter, timer }>`. `ctx.sessionId` (from the HTTP transport — echoed `mcp-session-id` header, else a generated UUID) is the key, so files persist across `codemode` calls within a session. Two eviction paths: (1) a per-session 15-minute idle timer (`IDLE_TTL_MS`, reset on every use, `unref`'d) — the guarantee, covering sessions that go quiet or whose client never sends a clean DELETE; (2) clean close — the transport calls `sessionManager.info.delete(id)` on client `DELETE /mcp`, and `src/codemode/session-eviction.ts` wraps the in-memory info manager (wired in `src/index.ts`) so that drops the sandbox — and the session's cached external MCP tool lists — immediately. `process.once('exit')` calls `disposeAllSandboxSessions()`. Nothing is written to disk. Cross-call persistence depends on the client echoing `mcp-session-id`; a client that doesn't gets a fresh session per call (harmless, TTL-reaped).
- **`sandbox` is in `RESERVED_NAMESPACES`** so neither a discovered service contextPath nor a header-supplied external MCP server name can shadow it.
- **Framed as a workspace, not a queried namespace — this altitude matters.** The sandbox is the agent's environment (where files live, where data is processed), NOT another API to discover. Framing it purely as a `describe()` peer led agents to treat it as an optional API they didn't need and dump files to chat instead of saving them. So it is surfaced at two levels: (1) `codemode.describe()` lists it (only when actually enabled for the run — `renderOverview`'s `sandboxEnabled` param mirrors whether `sandboxApi` was built) and `describe("sandbox")` returns the full interface (single constant `SANDBOX_INTERFACE_TS` in `sandbox/types.ts`, shared by describe + prompt + tool description), and (2) a **purpose** note ("this is where you save files / process data") in the always-read `codemode` tool description (`getSandboxNote`, rendered for `env === 'server'`) and in the `code-mode-guide` prompt (gated on `env === 'server' && custom.enableSandbox`, since the prompt handler runs per-request and can see the connection's actual opt-in state). The note is deliberately about _purpose_, not procedure. Earlier (pre-opt-in) attempts at procedural prose — guard with `typeof`, always `await`, not `bash` — made agents defensive and were reverted; now that the sandbox is genuinely absent for connections that have not opted in, the static tool description (built once per `env`, not per-connection) DOES tell the agent to check `typeof sandbox !== 'undefined'` before using it, same as an optional service namespace.
- **iso4 bridge type support (issue [#9](https://github.com/schplitt/iso4/issues/9), fixed in 0.3.x).** 0.3.x resolved the `Uint8Array` sandbox→host corruption, so `writeFile(path, <Uint8Array>)` and the `readFileBuffer` → `writeFile` round-trip work with no shim. `Date` is now _rejected_ by the wire codec (throws), so `stat().mtime` is deliberately converted to epoch **milliseconds** at the host-module leaf in `index.ts` (NOT in the Flue-pure adapter) before crossing the bridge — matched by `SANDBOX_INTERFACE_TS` declaring `mtime: number`. This is the only remaining bridge-driven deviation from Flue's `SandboxApi`.
...
- Prefer adding targeted tests near the affected behavior:
  ...
  - external MCP config parsing, namespace assembly, session tool-list cache: `test/external-mcp.test.ts`
  - full middleman flow incl. external servers over real HTTP: `test/e2e-mcp.test.ts`
...
- `@iso4/sandbox` (>= 0.6.0) is the execution boundary for `codemode`. It spawns a long-lived Rust child process on first use; `disposeSandbox()` shuts it down (called from the `excute.test.ts` `afterAll` hook and via a `process.once('exit')` handler in `execute.ts`). It provides `imports` (source modules + host modules) — the runtime is built on those instead of string preambles, and its `ResourceLimits` include `wallTimeMs` (default 30 s — must be raised explicitly when `cpuTimeMs` exceeds it). 0.3.x narrowed the bridge wire codec to reject unsupported built-ins (`Date`, `Map`, `Set`) loudly instead of silently corrupting them, and fixed `Uint8Array` sandbox→host transfer (see the sandbox surface notes).
- **0.6.0 migration notes.** Three things the release changed that touch this repo: `ERR_CAPACITY` is renamed `ERR_CAPACITY_MEMORY` (mc8yp never matched on it in code — only `openapi.json` prose named it), `SandboxStats` gains `slotLimit` (the currently derived concurrency, surfaced verbatim by `GET /stats`), and `cpuTimeMs` now measures CPU rather than elapsed time, so the 50 s cap in `SANDBOX_LIMITS` no longer fires on time a run spent waiting for a core — reported figures drop on a busy host and stop moving with how many runs share it. `wallTimeMs` (120 s) is what still bounds a starved run. `maxQueuedRuns` also changed default (flat `10_000` instead of `100 × maxConcurrentRuns`), which only matters if this repo ever starts pinning concurrency.
...
- `buildNamespaces(resolved, options?)` and `describeTarget(namespaces, methodIndex, target?, options?)` take OPTIONS OBJECTS, not positional policy arguments. Both accumulated per-connection knobs (restrictions, allow rules, `noMcp`, external servers / `sandboxEnabled`, `externalFailures`); adding the next one must not mean a fifth positional parameter.
- Connection-supplied external MCP servers are the only namespace source that is neither bundled nor tenant-discovered. When touching namespace assembly, discovery, or caching, check the External MCP Servers section above — the invariants that break silently are: tenant auth must never reach them, they must not enter the per-tenant method-index cache, and the tenant must win name collisions.

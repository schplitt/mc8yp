...
- **Framed as a workspace, not a queried namespace — this altitude matters.** The sandbox is the agent's environment (where files live, where data is processed), NOT another API to discover. Framing it purely as a `describe()` peer led agents to treat it as an optional API they didn't need and dump files to chat instead of saving them. So it is surfaced at two levels: (1) `codemode.describe()` lists it and `describe("sandbox")` returns the full interface (single constant `SANDBOX_INTERFACE_TS` in `sandbox/types.ts`, shared by describe + prompt + tool description), and (2) a **purpose** note ("this is where you save files / process data") in the always-read `codemode` tool description (`getSandboxNote`, rendered for `env === 'server'`) and in the `code-mode-guide` prompt. The note is deliberately about _purpose_, not procedure — earlier attempts at procedural prose (guard with `typeof`, always `await`, not `bash`) made agents defensive and were reverted.

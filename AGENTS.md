...
src/
  ...
  codemode/
    ...
    sandbox/
      ...
      session-eviction.ts     Server-only: InfoSessionManager wrapper that evicts a session's sandbox on clean client DELETE
  ctx/
...
- **One sandbox per MCP session, idle-evicted.** `index.ts` keeps a `Map<sessionId, { adapter, timer }>`. `ctx.sessionId` (from the HTTP transport — echoed `mcp-session-id` header, else a generated UUID) is the key, so files persist across `codemode` calls within a session. Two eviction paths: (1) a per-session 15-minute idle timer (`IDLE_TTL_MS`, reset on every use, `unref`'d) — the guarantee, covering sessions that go quiet or whose client never sends a clean DELETE; (2) clean close — the transport calls `sessionManager.info.delete(id)` on client `DELETE /mcp`, and `session-eviction.ts` wraps the in-memory info manager (wired in `src/index.ts`) so that drops the sandbox immediately. `process.once('exit')` calls `disposeAllSandboxSessions()`. Nothing is written to disk. Cross-call persistence depends on the client echoing `mcp-session-id`; a client that doesn't gets a fresh session per call (harmless, TTL-reaped).
- **`sandbox` is in `RESERVED_NAMESPACES`** so a discovered service contextPath can never shadow it.
